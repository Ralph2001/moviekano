...
export const SearchCard = ({ media, media_type }: TrendingCardProps) => {
  const [isFocus, setIsFocus] = useState<boolean>(false);
  const router = useRouter();

  const getMediaType = (media: MediaType): "movie" | "tv" | null => {
  ...
  const getMovieDetails = useCallback(
    (media: TMDBTypes.Trending) => {
      ...
      if (!mediaType) {
        console.warn("Unsupported media type:", media_type);
        return;
      }
  ...
  return (
    ...
    >
      {/* Image Container */}
      <div className="relative overflow-hidden w-full z-50  rounded-lg h-[20rem] transition-all duration-200 ease-in-out">
